...
import { minimumDays, maximumDays, isValid, fixedDay } from './delivery-date-default-jQueryUI.js';
import { dataTestMiniMumdayAndMaximumDay, dataArrayEmpty, dataString, dataNumber, dataObjEmpty, dataTestIsValid, specificProducts, specificCollections, settings, itemCart, currentCollection } from './dataTest.js';
...
describe('function fixedDay()', () => {
    ...
    it(`The result return new Date() = ${expectedOutput} if  specificProducts = [] and specificCollections = []`, () => {
        let res = fixedDay(itemCart, specificProducts, specificCollections, settings, currentCollection)
        let expectedOutput = new Date();
    ...
    it(`The result return new Date() = ${expectedOutput} if  cart item = []`, () => {
        let res = fixedDay(dataArrayEmpty, specificProducts, specificCollections, settings, currentCollection)
        let expectedOutput = new Date();
        res = JSON.stringify(res);
        expectedOutput = JSON.stringify(expectedOutput)
        chai.assert.equal(expectedOutput, res, `== new Date ${expectedOutput}`);
