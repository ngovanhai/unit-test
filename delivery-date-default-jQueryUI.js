...
export const isValid = (attributes, attributeNames, minimumDays, isEmpty) => {
  ...
  const currentDate = nowDate.setHours(0, 0, 0, 0);
...
export const fixedDay = (
  ...
  if (fixedDays.length > 0) {
    let fixedDays_in_milliseconds = fixedDays.map((day) => {
      if (day === "today") return nowDate.getTime();
      return new Date(day).getTime();
    });
    console.log("fixedDays_in_milliseconds", fixedDays_in_milliseconds);
    console.log(new Date(Math.max(...fixedDays_in_milliseconds)));
